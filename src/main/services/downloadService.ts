...
function requireFfprobePath(binaries: BinaryPaths): string {
  ...
  throw new BinaryMissingError(
    'ffprobe',
    join(dirname(binaries.ffmpeg), executableName('ffprobe'))
  );
}
  ...
  private emitProgress(progress: DownloadProgress): void {
    const progressWithLogPath =
      this.activeJob && !progress.logPath
        ...
        : progress;
    const decorated =
      this.activeJob?.decorateProgress?.(progressWithLogPath) ?? progressWithLogPath;
